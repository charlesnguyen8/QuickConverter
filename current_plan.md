...
**Goal:** every view is native React â€” no `dangerouslySetInnerHTML` bridges, no classic
...
changed â†’ `npm run build:ext` â†’ `npm test` green â†’ commit â†’ owner loads `dist/` for parity).
No piecemeal approval requests. Commit per milestone so each is independently revertible.

**Status (as of M1â€“M3):** Popup is fully React (`views/popup.js` + `popupMarkup.mjs` deleted); the
Settings DeepSeek tab is JSX and its controller lives in `components/settings-deepseek.js`. Next is
**M4** (Settings shell/tabs â†’ React, delete `views/settings.js`), then M5 Reader, M6 Novel, M7
...
1. **Load from `dist/`** â€” `npm run build:ext`, then load unpacked from `dist/`. Repo root is not
   loadable (React pages need the bundle).
2. **Static layers stay classic for now** â€” `services/`, `providers/`, and shared `components/*.js`
...
3. **Per-view entry** â€” `components/react/<view>-entry.jsx` mounts `<View>` into `#<view>-root`
   with `flushSync` (so sibling classic scripts that run at `DOMContentLoaded` still find DOM).
4. **No `chrome.*` in `views/` or React components** â€” go through `services/` (platform/storage).
5. **Reuse shared modules** â€” DeepSeek/AI config must stay the shared `AiConfigPanel`; do not
   duplicate provider UI.
6. **Mobile-first + Tailwind** â€” rebuilt `styles/tailwind.css` is committed; rerun
...
|---|---|---|
| Library | full (`LibraryView.jsx`) | â€” |
...
Tests: 12 suites green. Tests that grep HTML for moved IDs read the React/JSX module instead â€”
...
| Milestone | Commits |
|---|---|
| M1 Popup detail â†’ React | `e8e868c` (+ `ebe3b53` queue re-render, `deeb0f7` downloaded refresh, `f1b683b` null-novel guard) |
| M2 Popup DeepSeek card â†’ React, delete `popup.js` | `e083ef9`, `6d012f7` |
...
### M1 â€” Popup: detail view â†’ React â€” **done**
...
### M2 â€” Popup: DeepSeek panel + view deletion â€” **done**
...
### M3 â€” Settings: DeepSeek tab â†’ React (real JSX) â€” **done**
...
### M4 â€” Settings: shell + remaining tabs â†’ React â€” **done**
...
### M5 â€” Reader: finish the remaining pieces  â† **next** (5a)
`views/reader.js` is ~701 lines with coupled subsystems; split into four slices. Each slice: convert
markup with the same IDs, keep `reader.js` wiring (React renders once, so DOM refs stay valid â€”
the bridge pattern used for the popup DeepSeek card), build + 13 suites + a `dist/` check.

- **5a â€” Header markup â€” done**: `components/react/ReaderHeader.jsx` + `reader-header-entry.jsx`
  (flushSync); `views/reader.html` lines 469â€“794 replaced with `#reader-header-root`; `reader.js`
  unchanged (bridge pattern). Tests read `ReaderHeader.jsx` for the moved ids/options.
- **5b â€” Typography popover logic â†’ React**: `ReaderPrefsPopover.jsx` owns the popover state and
...
- **5c â€” Translation panel + source drawer**: render the `AiConfigPanel` container from React (same
  IDs) and move `initDeepSeekUI` wiring; hook the source drawer to React state.
- **5d â€” Chapter load / not-saved / download flow + toast + hotkeys**: port the remaining
...
### M6 â€” Novel: finish the remaining pieces
...
### M8 â€” (deferred) ESM + MV3 bundling
...
1. **OAuth under `dist/`** â€” dist extension ID differs from the registered client; register a second
   "Chrome Extension" OAuth client (or re-register) so cloud sync works in `dist/`.
2. **Scraper** â€” `wetriedtls` serves a Cloudflare interstitial; content script logs "Could not find
